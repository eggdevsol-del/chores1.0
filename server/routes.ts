...
// Helper to get the authenticated user ID from Passport session
...
export function registerRoutes(app: Express) {
  ...
  // Legacy register route - redirects to new auth
  app.post("/api/auth/register", async (_req, res) => {
    return res.status(410).json({ error: "Use /auth/register instead" });
  });

  // Legacy login route - redirects to new auth
  app.post("/api/auth/login", async (_req, res) => {
    return res.status(410).json({ error: "Use /auth/login instead" });
  });

  // Logout - works with Passport sessions
  ...
  // Get current user - works with Passport sessions
  ...
  app.get("/api/kids", async (req, res) => {
    ...
    try {
      const kids = await getKidsByUserId(userId);
      res.json(kids);
    } catch (error) {
  ...
  app.post("/api/chores/:choreId/assign", async (req, res) => {
    ...
    try {
      ...
      if (assignToAll) {
        await assignChoreToAllKids(choreId, userId);
      } else if (kidId) {
        await assignChoreToKid(choreId, kidId, userId);
      } else {
